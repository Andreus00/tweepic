...
def create_online_sentence_proximity_pipeline(n_neighbors=5):
    online_distance_calculator = OnlineDistanceCalculator(n_neigh=n_neighbors)
    return Pipeline(stages=[online_distance_calculator])
...
class CrossJoin(Transformer):
    ...
    def _transform(self, df: DataFrame):
        ...
        df_bucket = df.filter((df["time_bucket"] == first_bucket) | (df["time_bucket"] == first_bucket + 1))
        df2 = df_bucket.select(F.col("id").alias("id_2"), F.col("text").alias("text_2"), "time_bucket", F.col("sentence_embeddings").alias("sentence_embeddings_2"), F.col("word_embeddings").alias("word_embeddings_2"), F.col("hashtags_embeddings").alias("hashtags_embeddings_2"))
        df_bucket = df_bucket.join(df2, (df_bucket["time_bucket"] == df2["time_bucket"]) & (df_bucket["id"] != df2["id_2"]), how="inner")
        df_final = df_bucket.select("id", "text", "sentence_embeddings", "word_embeddings", "hashtags_embeddings", "id_2", "text_2", "sentence_embeddings_2", "word_embeddings_2", "hashtags_embeddings_2")


        for bucket in buckets[1:]:
            df_bucket = df.filter((df["time_bucket"] == bucket["time_bucket"]) | (df["time_bucket"] == bucket["time_bucket"] + 1) | (df["time_bucket"] == bucket["time_bucket"] - 1))
            df2 = df_bucket.select(F.col("id").alias("id_2"), F.col("text").alias("text_2"), "time_bucket", F.col("sentence_embeddings").alias("sentence_embeddings_2"), F.col("word_embeddings").alias("word_embeddings_2"), F.col("hashtags_embeddings").alias("hashtags_embeddings_2"))
            df_bucket = df_bucket.join(df2, (df_bucket["time_bucket"] == df2["time_bucket"]) & (df_bucket["id"] != df2["id_2"]), how="inner")
            df_bucket = df_bucket.select("id", "text", "sentence_embeddings", "word_embeddings", "hashtags_embeddings", "id_2", "text_2", "sentence_embeddings_2", "word_embeddings_2", "hashtags_embeddings_2")
            df_final = df_final.union(df_bucket)
        ...
        return df_final.select("id", "text", "sentence_embeddings", "word_embeddings", "hashtags_embeddings", "id_2", "text_2", "sentence_embeddings_2", "word_embeddings_2", "hashtags_embeddings_2")
